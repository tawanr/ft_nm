...
void get_symtab_info(Elf_File64 *elf_file, Elf_Symtab64 *info, const unsigned int section_type)
{
    info->symtab = NULL;
    for (int i = 0; i < elf_file->file_headers->e_shnum; i++)
    {
        ...
        {
            info->symtab = (Elf64_Sym *)(elf_file->file +
                                         elf_file->section_headers[i].sh_offset);
            info->symcount = elf_file->section_headers[i].sh_size / elf_file->section_headers[i].sh_entsize;
            info->symstr = (char *)(elf_file->file +
                                    elf_file->section_headers[elf_file->section_headers[i].sh_link].sh_offset);
            return;
        }
    }
}
...
Elf_Symbol **extract_symbols64(Elf_File64 *elf_file)
{
    ...
    for (int i = 0; i < info.symcount; i++)
    {
        ...
        if (info.symtab[i].st_shndx < elf_file->file_headers->e_shentsize)
            symbols[i]->section_rel = elf_file->section_headers[info.symtab[i].st_shndx].sh_type;
    }
    return symbols;
}
...
void parse_file64(int fd, Elf_File64 *elf_file)
{
    ...
    if (elf_file->file_size <= 0)
        return;
    elf_file->file = mmap(0, elf_file->file_size, PROT_READ, MAP_PRIVATE, fd, 0);
    if (elf_file == NULL)
        return;
    ...
    if (elf_file->file_headers->e_shoff > elf_file->file_size ||
        ...
        return;
    elf_file->section_headers = (Elf64_Shdr *)(elf_file->file + elf_file->file_headers->e_shoff);
